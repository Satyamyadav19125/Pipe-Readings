...
// Filter out submissions belonging to disabled farms/pipes (for use in the
// red-flag pass and coverage so disabled units never flag or count as missed).
export async function excludeDisabled(submissions) {
  const reg = await getDisabledRegistry();
  const offFarms = new Set((reg.farms || []).map(lc));
  const offPipes = new Set((reg.pipes || []).map(lc));
  if (offFarms.size === 0 && offPipes.size === 0) return submissions;
  return submissions.filter((s) =>
    !offFarms.has(lc(getField(s, 'farm'))) && !offPipes.has(lc(getField(s, 'serial'))));
}
