...
export async function filterSubmissionsForUser(submissions) {
  ...
  // Guest (read-only viewer): NEVER real data — a self-contained safe demo set
  // sized to the admin's "max readings". This keeps every guest view populated
  // (villages, flags, consumption, duplicates, map) without exposing anything.
  if (user.role === 'guest') return getDemoSubmissions(user.maxReadings || 10);
