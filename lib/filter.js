...
export async function filterSubmissionsForUser(submissions) {
  ...
  // Guest (read-only viewer) sees the full project like an admin; the page
  // then caps how many rows are shown so no bulk data leaves the tool.
  if (user.role === 'guest') return submissions;
