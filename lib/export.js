...
// Date, Surveyor, Village, Farm ID, Pipe ID, Outside & Inside water level,
// Longitude, Latitude, and the submission's unique ID.
export function toCleanLabeledRows(submissions) {
  return submissions.map((s) => {
    ...
    return {
      ...
      'Pipe ID': getField(s, 'serial') ?? '',
      'Outside water level (mm)': getField(s, 'validation') ?? '',
      'Inside water level (mm)': getField(s, 'endReading') ?? getField(s, 'reading') ?? '',
      'Longitude': lng,
      'Latitude': lat,
...
function prettyPath(path) {
  ...
  const map = {
    Readings_mm: 'Water level (mm)', Outside_validation: 'Outside height (mm)',
    pipes: 'Pipe ID', farm: 'Farm ID', village: 'Village', name: 'Surveyor',
    Date: 'Date', Location: 'GPS',
  };
  return map[seg] || seg.replace(/_/g, ' ');
}
...
export function toCorrectedChangeRows(submissions, rawById = new Map()) {
  ...
  for (const s of submissions) {
    ...
    const base = { 'Submission UID': s._id, 'Pipe ID': getField(raw, 'serial') ?? getField(s, 'serial') ?? '', 'Village': getField(raw, 'village') ?? '' };
    if (c.field === 'fields' && c.fields && typeof c.fields === 'object') {
      ...
      rows.push({ ...base, 'Field changed': 'Water level (mm)', 'Old value (raw Kobo)': c.oldValue ?? '', 'New value (corrected)': c.newValue ?? '', 'Reason': c.note || '', 'By': c.by || '', 'When': when });
    }
  }
  return rows;
}
