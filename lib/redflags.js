...
export function detectRedFlags(submissions, options = {}) {
  ...
  // Pipe physical parameters (mm) from Settings -> Pipe parameters. The two
  // range rules only run when the relevant numbers are actually configured.
  const maxInside = Number(pipe.maxInsideMm);
  const outsideMin = Number(pipe.outsideMinMm);
  const outsideMax = Number(pipe.outsideMaxMm);
  ...
  for (const serial in groups) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      // Inside water-level reading outside the physically possible range
      if (isOn('inside_out_of_range') && Number.isFinite(maxInside) && !Number.isNaN(endR)) {
        if (endR < 0 || endR > maxInside) {
          push(id, serial, { type: 'inside_out_of_range', severity: 'high', message: `Inside reading ${endR} mm is outside the valid 0–${maxInside} mm range set in Settings → Pipe parameters` });
        }
      }

      // Outside pipe height (the validation measurement) outside the expected band
      if (isOn('outside_out_of_range') && Number.isFinite(outsideMin) && Number.isFinite(outsideMax)) {
        const outsideR = parseReading(getField(sub, 'validation'));
        if (!Number.isNaN(outsideR) && (outsideR < outsideMin || outsideR > outsideMax)) {
          push(id, serial, { type: 'outside_out_of_range', severity: 'medium', message: `Outside height ${outsideR} mm is outside the expected ${outsideMin}–${outsideMax} mm band — pipe may have sunk, lifted, or been re-installed` });
        }
      }
