...
export const ALL_FLAG_TYPES = [
  ...
  { key: 'stale_no_reading', label: 'Stale (no reading within the current reading period)', severity: 'medium' },
...
export function detectRedFlags(submissions, options = {}) {
  const {
    ...
    geofence = null, refLocations = null, maxLocationKm = 100,
  } = options;
  ...
  for (const serial in groups) {
    ...
    if (list.length > 0) {
      ...
      if (isOn('stale_no_reading') && daysSince > staleDays) {
        push(id, serial, { type: 'stale_no_reading', severity: 'medium', message: `No reading taken for ${daysSince.toFixed(0)} days on pipe ${serial} (the reading period is ${staleDays} day${staleDays === 1 ? '' : 's'})` });
      }
