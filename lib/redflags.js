  ...
  for (const serial in groups) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      if (isOptIn('invalid_meter_id')) {
        if (!/^[A-Z]{2}_\d{3,}[A-Z]?$/i.test(String(serial))) {
          push(id, serial, { type: 'invalid_meter_id', severity: 'low', message: `Pipe ID "${serial}" doesn't match the expected AB_00000 format` });
        }
      }
