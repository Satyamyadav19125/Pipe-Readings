...
export const ALL_FLAG_TYPES = [
  ...
  { key: 'reading_jump_up', label: 'Water level jumped up vs the previous reading (e.g. 150 → 300)', severity: 'medium' },
  ...
  { key: 'geofence', label: 'Reading taken outside the pipe\'s set location radius', severity: 'medium' },
...
// offline — so every date-based rule must use the form date first.
...
// Same-day duplicates: the SAME pipe (on the same farm) read more than once on
// the same field DATE. Returns a map id -> [other ids] and a Set of all ids
// involved, so the UI can show both forms side-by-side for review regardless of
// whether the duplicate red-flag toggle is on. Dead readings are ignored.
export function sameDayDuplicates(submissions) {
  const groups = {};
  for (const sub of submissions) {
    // NOTE: dead (deleted) readings are intentionally INCLUDED so a pair the
    // admin has already resolved (one marked dead) still shows as a duplicate —
    // you can see which one you deleted and which one you kept.
    const serial = getField(sub, 'serial');
    if (!serial) continue;
    const t = readingTime(sub);
    if (!t) continue;
    const farm = getField(sub, 'farm') || '';
    const key = `${farm}|${serial}|${new Date(t).toISOString().slice(0, 10)}`;
...
export function groupBySerial(submissions) {
  ...
  for (const serial in groups) {
    groups[serial].sort((a, b) => readingTime(a) - readingTime(b));
  }
  return groups;
}
...
function parseLocForGeo(v) {
  if (!v) return null;
  if (Array.isArray(v) && v.length >= 2) return { lat: Number(v[0]), lng: Number(v[1]) };
...
export function detectRedFlags(submissions, options = {}) {
  const {
    ...
    geofence = null, refLocations = null, maxLocationKm = 100, flagWindowDays = null,
  } = options;
  ...
  if (isOptIn('fabrication_speed')) {
    ...
    for (const sub of submissions) {
      ...
      // Compare upload gaps only WITHIN the same field day, otherwise a bulk
      // upload after days offline looks like impossibly fast data entry.
  ...
  // almost certainly swapped lat/long or mistyped a coordinate (e.g. Sidhuwal's
  // "30.37 30.37" landing in Egypt). Median is used so the bad points can't
  // drag the centre; a single wild outlier still trips.
  ...
  for (const serial in groups) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      if (isOptIn('reverse') && !Number.isNaN(startR) && !Number.isNaN(endR) && startR !== endR && endR < startR) {
      ...
      // Geofence: reading taken too far from this pipe's reference location.
      if (geofence && geofence.enabled && refLocations) {
        const ref = refLocations[serial] || refLocations[String(serial).toUpperCase()];
        if (ref) {
          const loc = parseLocForGeo(getField(sub, 'location')) || parseLocForGeo(sub._geolocation);
          if (loc) {
            const dist = distanceMeters(ref, loc);
            const radius = Number(geofence.radiusMeters) || 50;
            if (dist > radius) {
              push(id, serial, { type: 'geofence', severity: 'medium', message: `Reading taken ${dist.toFixed(0)} m from this pipe's set location — beyond the ${radius} m radius. Possibly read at the wrong pipe.` });
            }
          }
        }
      }
      ...
      if (isOptIn('duplicate_same_day')) {
        // Use the FORM date (when the reading was actually taken), not the
        // upload time. Readings taken on different days are often uploaded
        // together when a phone syncs late — that is NOT a duplicate.
        const t = readingTime(sub);
        if (!Number.isNaN(t)) {
          // Key on farm+pipe+day so two DIFFERENT pipes on the same farm on the
          // same day are never wrongly flagged as duplicates. Only the exact
          // same pipe read twice in a day trips this.
          const farm = getField(sub, 'farm') || '';
          const dayKey = `${farm}|${serial}|${new Date(t).toISOString().slice(0, 10)}`;
          if (seenDays[dayKey]) {
            push(id, serial, { type: 'duplicate_same_day', severity: 'low', message: `This exact pipe (${serial}) was already read on ${new Date(t).toISOString().slice(0, 10)}`, previousSubmissionId: seenDays[dayKey], previousDate: new Date(t).toISOString().slice(0, 10) });
          }
          // Remember the submission id for this pipe+day so the duplicate can
          // link back to it for the side-by-side comparison view.
          seenDays[dayKey] = id;
        }
      }
      ...
      if (i > 0) {
        ...
        if (!Number.isNaN(prevEnd) && !Number.isNaN(endR)) {
          if (isOptIn('rollback') && endR < prevEnd) {
          ...
          } else if (isOptIn('huge_jump') && endR - prevEnd > hugeJumpThreshold) {
          ...
          } else if (isOptIn('growth_anomaly') && med !== null && daysGap > 0) {
          ...
          // Water level jumped UP a lot vs the previous reading (e.g. 150 -> 300).
          // Opt-in: in AWD the level can legitimately rise after irrigation, so
          // the admin turns this on when a doubling is worth reviewing. Fires on
          // a roughly-doubled reading that isn't already caught as a huge jump.
          if (isOptIn('reading_jump_up') && endR > prevEnd
              && endR >= prevEnd * 2 && (endR - prevEnd) >= 50
              && (endR - prevEnd) <= hugeJumpThreshold) {
            push(id, serial, { type: 'reading_jump_up', severity: 'medium', message: `Water level jumped up from ${prevEnd} to ${endR} vs the previous reading (roughly doubled). Check it's not a misread.`, previousReading: prevEnd, previousSubmissionId: prev._id, previousDate: prev._submission_time, currentReading: endR });
          }
    ...
    if (list.length > 0) {
      ...
      // "No reading recently" deliberately does NOT raise a red flag any more —
      // it is tracked in the Assignment / missed-readings tab (period-based), so
      // a legitimate gap spanning a week boundary (e.g. Monday one week, Saturday
      // the next) is never wrongly flagged. `daysSince`/`staleDays` are kept for
      // the timeline/other callers.
      void daysSince;
  ...
  // Review window: drop flags on OLD readings (taken more than N days ago), so
  // a reading nobody has touched in weeks isn't kept in the red-flag list. Blank
