...
export const DEFAULT_SETTINGS = {
  ...
  redFlags: {
    ...
    // "No reading recently" is tracked in the Assignment / missed-readings tab
    // (period-based), NOT as a red flag — so a legit gap across a week boundary
    // is never flagged. Off here; opt-in if an admin wants it as a flag too.
    stale_no_reading: false,
    // ON: no photo on a submission.
    missing_photo: true,
  ...
  guest: {
    ...
    show: {                 // which sections a guest may open
      ...
      assignment: true, team: true, redFlags: true, charts: true,
    },
    landing: {              // the guest landing page (item 13/14)
      projectName: '',      // blank = use the project name
      tagline: 'Water-level monitoring dashboard',
      description: '',      // blank = a generic line (no PVC / AWD / universities)
